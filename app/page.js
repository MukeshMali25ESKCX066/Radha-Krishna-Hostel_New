...
export default function Home() {
  ...
  return <main>
    <header><div className="nav"><a className="logo" href="#home"><span><b>Radha Krishan Hostel</b><small>राधा कृष्ण हॉस्टल</small></span></a><nav>{['Home', 'Rooms', 'Availability', 'Floor Plan', 'Facilities', 'Gallery', 'Location', 'Contact'].map((item) => <a key={item} href={`#${item.toLowerCase().replace(' ', '-')}`}>{item}</a>)}</nav><div className="portal-links"><a href="/student-login">Student Login</a><a href="/register">Register</a><a href="/admin-login">Admin Login</a></div><a className="wa-nav" href={whatsappUrl} target="_blank" rel="noreferrer">WhatsApp</a></div></header>
    <section id="home" className="hero"><div className="shade" /><div className="hero-text"><label>STUDENT & PARENT FRIENDLY HOSTEL</label><h1>Your Comfortable<br /><em>Home Away From Home</em></h1><p>Welcome to Radha Krishan Hostel, Jaipur — a comfortable, convenient and student-friendly place to stay.</p><div><a className="btn primary" href="#rooms">View Rooms →</a><a className="btn secondary" href="/student-login">Student Login</a></div><small>📍 Ramnagariya, Jaipur, Rajasthan</small></div></section>
    ...
    <section id="gallery" className="section beige center"><label>HOSTEL GALLERY</label><h2>Take a Look Around</h2><p className="desc">A comfortable, clean and welcoming place to stay in Jaipur.</p><div className="gallery">{galleryImages.map((image) => <img key={image.src} src={image.src} alt={image.alt} loading="lazy" />)}</div></section>
